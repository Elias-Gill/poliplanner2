...
import (
	...
	course "github.com/elias-gill/poliplanner2/internal/domain/courseOffering"
...
// FIX: completar bien los campos sql como debe de ser
func (s *SqliteCoursesStore) FindById(ctx context.Context, id int64) (*course.CourseOffering, error) {
...
func (s *SqliteCoursesStore) ListByCareerAndPeriod(
	...
	itemsMap := make(map[int64]*course.CourseListItem)
	...
	for rows.Next() {
		...
		if teacherID != nil {
			item.Teachers = append(item.Teachers, course.Teacher{
				Name:  *teacherName,
				Email: *teacherEmail,
			})
		}
	...
	items := make([]*course.CourseListItem, 0, len(itemsMap))
...
func (s *SqliteCoursesStore) Upsert(
	ctx context.Context,
	insertFn func(persist func(course.CourseAggregate) error) error,
) error {
	...
	persist := func(grade course.CourseAggregate) error {
...
func (s *SqliteCoursesStore) upsertPeriod(tx *sql.Tx, ctx context.Context, p course.Period) (int64, error) {
	var id int64
...
func (s *SqliteCoursesStore) upsertTeachers(tx *sql.Tx, ctx context.Context, teachers []course.Teacher) ([]int64, error) {
...
func (s *SqliteCoursesStore) upsertSingleTeacher(tx *sql.Tx, ctx context.Context, teacher course.Teacher) (int64, error) {
...
func (s *SqliteCoursesStore) upsertCurriculum(tx *sql.Tx, ctx context.Context, c course.Curriculum) (int64, error) {
...
func (s *SqliteCoursesStore) upsertCourse(tx *sql.Tx, ctx context.Context, grade course.CourseAggregate, mallaID, periodID int64) (int64, error) {
	var id int64
	err := tx.QueryRowContext(ctx, `
        INSERT INTO cursos (
...
// FIX: que use el nuevo modelo con DDD correcto. 
// Esto es un read model, que deberia de hacer el join con el teacher tambien. Tal vez ver que
// partes del subject no son necesarias para la app.
func scanCourseModel(row *sql.Row) (*course.CourseOffering, error) {
	gm := &course.CourseOffering{}
...
func (s *SqliteCoursesStore) loadTeachersForCourse(ctx context.Context, courseID int64, gm *course.CourseAggregate) error {
	rows, err := s.db.QueryContext(ctx, `
		SELECT d.nombre
	...
	gm.Teachers = make([]course.Teacher, len(names))
